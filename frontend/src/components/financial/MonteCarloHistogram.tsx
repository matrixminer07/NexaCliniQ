  ...
  return (
    ...
    >
      <div className="flex justify-between items-center px-1 mb-4">
        <div className="text-center"><Text className="text-[10px] text-gray-400 block uppercase tracking-wider mb-0.5">P10 (Downside)</Text><span className="font-mono text-red-500 font-semibold text-sm">{formatMillions(p10)}</span></div>
        <div className="text-center"><Text className="text-[10px] text-blue-400 block uppercase tracking-wider mb-0.5">P50 (Expected)</Text><span className="font-mono text-blue-600 font-bold text-lg">{formatMillions(p50)}</span></div>
        <div className="text-center"><Text className="text-[10px] text-gray-400 block uppercase tracking-wider mb-0.5">P90 (Upside)</Text><span className="font-mono text-emerald-500 font-semibold text-sm">{formatMillions(p90)}</span></div>
      </div>
      <div style={{ width: '100%', height: 300, marginTop: 12 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="npv" tickFormatter={(val) => formatMillions(Number(val), 0)} tick={{ fontSize: 10, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
            <Tooltip formatter={(val: any) => [val, 'Scenarios']} labelFormatter={(l) => `NPV: ${formatMillions(Number(l))}`} />
